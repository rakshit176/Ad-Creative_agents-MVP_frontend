...
import getFile from "../Sam/helpers/getFile";
import { handleImageScale } from "../Sam/helpers/ImageHelper";
import { modelScaleProps } from "../Sam/helpers/Interface";
import { getAllMasks, keepArrayForMultiMask, rleToImage, traceCompressedRLeStringToSVG, traceOnnxMaskToSVG } from "../Sam/helpers/mask_utils";
import { modelData, setParmsandQueryEraseModel, setParmsandQueryModel } from "../Sam/helpers/modelAPI";
import AppContext from "../Sam/hooks/createContext";
import Stage from "../Sam/Stage";
...
function SamCanvas() {
  ...
  useEffect(() => {
    const initModel = async () => {
      try {
        ...
        console.log("MODEL:", e); //test
        console.error(e);
      }
      try {
        // console.log("MULTI MASK MODEL");
        // if (process.env.MULTI_MASK_MODEL_DIR === undefined) return;
        const MULTI_MASK_MODEL_DIR = "./interactive_module_quantized_592547_2023_03_19_sam6_long_uncertain.onnx";
  ...
  const runMultiMaskModel = async () => {
    try {
      ...
      const feeds = modelData({ clicks, tensor, modelScale, last_pred_mask: null, });// Only 1 click allowed, so no last predicted mask exists 
  ...
  return (
    <div className={`container-fluid w-[95vw] bg-canvasBg`}>
