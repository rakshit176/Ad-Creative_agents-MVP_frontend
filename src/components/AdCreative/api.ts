...
const batchCall = (asyncFunction: { (): Promise<any>; (arg0: any): Promise<any>; }) => {
...
let subDomainCache: string | null = null;
const getPublicSubDomain = batchCall(async () => {
  ...
  const existingDomain = sites.find(
    (site: { subdomain: string | string[]; }) => site.subdomain.indexOf(prefix) >= 0
  );
...
export const deleteAsset = async ({ id }) => {
  const list = await listAssets();
  const newList = list.filter((asset: { id: any; }) => asset.id !== id);
